...
public class RecuitKP {
	...
	// BELDJILALI - FEJRI - 16 novembre 2018 - IENAC
	// *************************************************************
	// r�ponse question knapsack
	// la valeur V = somme des xi*vi est maximois�e
	// s.t. P = somme des xi*pi < C , C capacit� du sac
	// crit�re pour relaxation :
	...
	public double chauffage() {
		...
		do {
			nbAccept = 0;
			for (int i = 0; i < nbTransitions; i++) {// generation d'un point de l'espace d'etat
				xi.initAleatEtat();
				yi = xi.calculCritere();
	...
	public EtatKP refroidissement(double Tinit) {
		...
		do {
			...
			for (int i = 0; i < nbTransitions; i++) {// palier à la temperature T qui dure le "temps" de nbTransitions
				xi.genererVoisin();
				yj = xi.calculCritere();
				if (accept(yi, yj, T, minimisation)) {
					yi = yj;
			...
			T = T * alpha; // refroidissement alpha = 0.995
